...
- Added app-specific validation for credential payloads: SoftMouse requires `username`/`password`; Fair3r, ElabFTW, Precliniset, ProtocolIo, and Tecniplast require `apiKey`, `token`, or `accessToken`.
...
- A single item can now carry multiple external IDs (UUID in MAPP, ID in SoftMouse, ID from Fair3r, etc.) through structured credential storage per connected app.
...
- Horizon impact: Medium term — needed as more external systems (OSF, SoftMouse, Tecniplast) are integrated.
