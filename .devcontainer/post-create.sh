...
sudo apt-get update
sudo apt-get install -y --no-install-recommends curl dnsmasq ipset iptables jq unzip
...
if ! command -v castor >/dev/null 2>&1; then
...
if ! command -v dcode >/dev/null 2>&1; then
  uv tool install "deepagents-code==${DEEPCODE_VERSION}"
fi
