...
host_gateway_ip="$(getent ahostsv4 host.docker.internal 2>/dev/null | awk '{ print $1; exit }' || getent hosts host.docker.internal | awk '/^[0-9.]+/ { print $1; exit }')"
...
domains=(
  ...
  sentry.io
  statsig.com
  # uv / Python tooling — post-create.sh installs uv before this firewall runs,
  # but agents or scripts that call uv after startup need these.
  pypi.org
  files.pythonhosted.org
  # Note: Docker Hub (docker.io) is intentionally absent. The Docker socket is
  # bind-mounted from the host, so image pulls go through the host daemon's
  # network namespace and are not subject to this devcontainer firewall.
)
