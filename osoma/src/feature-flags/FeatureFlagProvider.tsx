...
export function FeatureFlagProvider({ children }: { children: React.ReactNode }) {
  ...
  React.useEffect(() => {
    // The overrides endpoint requires auth (ROLE_USER); fetching before the
    // session is ready 401s and never refetches. Wait for auth to settle, then
    // load — anonymous users just fall back to compiled defaults.
    if (isLoading) return
    if (!isAuthenticated) {
