...
import { Send, Bot, User, Sparkles, Loader2, AlertCircle, Wrench } from 'lucide-react'
import { getAiStatus, sendAiChat, sendAgenticChat } from '../assistant.api'
import { callMcpTool } from '../api/mcpApi'
import type { AiChatMessage } from '../assistant.types'
import { AiStatusBadge } from './AiStatusBadge'
import { useFeatureFlagContext } from '@/feature-flags/FeatureFlagProvider'
import { Button } from '@/components/ui/button'
import { Input } from '@/components/ui/input'
import { Avatar, AvatarFallback } from '@/components/ui/avatar'

/** A single tool the agentic loop invoked, surfaced as a compact trail. */
type ToolStep = { tool: string; success: boolean }
/** Chat message, optionally carrying the agentic tool-call trail. */
type ChatMessage = AiChatMessage & { tools?: ToolStep[] }

const SUGGESTIONS = [
  'How can you help with metadata review?',
  'Standardize and harmonize my reference library',
  'Harmonize glucose measures across sources',
...
// Cross-source reference lookups in external connected databases (not internal entities).
const REFERENCE_INTENT_PATTERN =
  /\b(reference|standardi[sz]|ontolog|\bcde\b|common data element|phenotyp|procedure|protocol|blood sampling|sampling (?:technique|method)|strain|cross[- ]?reference)\b/
...
const resolveMcpToolRequest = (content: string): McpToolRequest | null => {
  ...
  // Federated reference search across connected external databases.
  if (REFERENCE_INTENT_PATTERN.test(lowercaseMessage)) {
    const query = content
      .replace(/^\s*(find|search(?:\s+for)?|look\s?up|lookup|show me|get me|locate)\s+(?:a|an|the|me)?\s*/i, '')
      .trim()
    return { name: 'search_reference_resources', args: { query: query || content.trim() } }
  }
...
function ToolTrail({ steps }: { steps: ToolStep[] }) {
  return (
    <div className="mt-2 flex flex-wrap items-center gap-1.5 border-t border-slate-100 pt-2" data-testid="agentic-tool-trail">
      <span className="text-[10px] uppercase tracking-wide text-slate-400">Tools</span>
      {steps.map((step, index) => (
        <span
          key={`${step.tool}-${index}`}
          className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[10px] font-medium ${
            step.success ? 'bg-slate-100 text-slate-600' : 'bg-red-50 text-red-600'
          }`}
        >
          <Wrench className="h-2.5 w-2.5" /> {step.tool} {step.success ? '✓' : '✕'}
        </span>
      ))}
    </div>
  )
...
export function AssistantChat() {
  ...
  const agenticEnabled = isEnabled('feature.aiAssistant.agentic')
  ...
  const agenticMutation = useMutation({
    mutationFn: (nextMessages: AiChatMessage[]) => sendAgenticChat({ messages: nextMessages }),
  })
  const isResponding = chatMutation.isPending || agenticMutation.isPending || isToolLoading
  ...
  const handleSend = async (content: string) => {
    ...
    // Agentic mode: let the backend's bounded tool-use loop orchestrate the MCP
    // tools and return a tool-call trail, instead of single-shot keyword routing.
    if (agenticEnabled) {
      try {
        const response = await agenticMutation.mutateAsync(newMessages)
        const tools: ToolStep[] = (response.provenance ?? [])
          .filter((entry) => typeof entry.tool === 'string')
          .map((entry) => ({ tool: entry.tool as string, success: entry.success !== false }))
        setMessages([
          ...newMessages,
          { role: 'assistant', content: normalizeAssistantReply(response.reply), tools: tools.length > 0 ? tools : undefined },
        ])
      } catch (error) {
        ...
        {messages.map((message, index) => (
          ...
          >
            ...
            >
              ...
              {message.tools && message.tools.length > 0 ? <ToolTrail steps={message.tools} /> : null}
            </div>
          </div>
        ))}
        {isResponding && (
