...
const { getAiStatusMock, sendAiChatMock, sendAgenticChatMock, callMcpToolMock, flagState } = vi.hoisted(() => ({
  getAiStatusMock: vi.fn(),
  sendAiChatMock: vi.fn(),
  sendAgenticChatMock: vi.fn(),
  callMcpToolMock: vi.fn(),
  flagState: { agentic: false },
}))

vi.mock('../assistant.api', () => ({
  getAiStatus: getAiStatusMock,
  sendAiChat: sendAiChatMock,
  sendAgenticChat: sendAgenticChatMock,
}))
...
vi.mock('@/feature-flags/FeatureFlagProvider', () => ({
  useFeatureFlagContext: () => ({ isEnabled: (key: string) => (key === 'feature.aiAssistant.agentic' ? flagState.agentic : false) }),
}))
...
describe('AssistantChat', () => {
  ...
  it('routes through the agentic endpoint and renders the tool-call trail when the flag is on', async () => {
    const user = userEvent.setup()
    flagState.agentic = true
    sendAgenticChatMock.mockResolvedValue({
      reply: 'I imported the references you asked for.',
      status: {
        enabled: true, available: true, configured: true, realProvider: false,
        provider: 'mock', message: 'Using mock AI gateway.', capabilities: ['agentic_chat'],
      },
      trace: { traceId: 'trace-agentic', action: 'agentic_chat', route: '/ai/agentic-chat', recordedAt: '2026-03-24T00:00:00Z' },
      provenance: [
        { label: 'MCP tool call', value: 'list_my_references', source: 'mcp:list_my_references', tool: 'list_my_references', success: true },
        { label: 'MCP tool call', value: 'import_reference', source: 'mcp:import_reference', tool: 'import_reference', success: true },
      ],
      warnings: [],
      structuredOutput: {},
    })
    ...
    await user.type(screen.getByPlaceholderText(/Ask how to use AI-assisted review/i), 'Import my glucose references')
    await user.click(screen.getByRole('button', { name: /send/i }))

    await waitFor(() => expect(sendAgenticChatMock).toHaveBeenCalled())
    expect(sendAiChatMock).not.toHaveBeenCalled()
    expect(callMcpToolMock).not.toHaveBeenCalled()
    expect(await screen.findByText(/I imported the references you asked for/i)).toBeInTheDocument()

    const trail = await screen.findByTestId('agentic-tool-trail')
    expect(trail).toHaveTextContent('list_my_references')
    expect(trail).toHaveTextContent('import_reference')
  })

  it('harmonizes references from a suggested prompt via the agentic loop', async () => {
    const user = userEvent.setup()
    flagState.agentic = true
    sendAgenticChatMock.mockResolvedValue({
      reply: 'I linked your references and harmonized the fields.',
      status: {
        enabled: true, available: true, configured: true, realProvider: false,
      ...
      provenance: [
        { label: 'MCP tool call', value: 'list_my_references', source: 'mcp:list_my_references', tool: 'list_my_references', success: true },
        { label: 'MCP tool call', value: 'standardize_references', source: 'mcp:standardize_references', tool: 'standardize_references', success: true },
      ],
      warnings: [],
      structuredOutput: {},
    })
    ...
    renderChat()
    await user.click(screen.getByRole('button', { name: /Standardize and harmonize my reference library/i }))

    await waitFor(() => expect(sendAgenticChatMock).toHaveBeenCalled())
    expect(await screen.findByText(/harmonized the fields/i)).toBeInTheDocument()
    const trail = await screen.findByTestId('agentic-tool-trail')
    expect(trail).toHaveTextContent('standardize_references')
  })
