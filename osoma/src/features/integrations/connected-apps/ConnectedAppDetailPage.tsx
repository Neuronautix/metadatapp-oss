...
import {
    ...
    testImpcConnection,
    testJaxPhenomeConnection,
    testNihCdeConnection,
    testOsfConnection,
...
import { SoftMouseImportTab } from './components/SoftMouseImportTab.tsx'
...
import { NihCdeBrowserPanel } from './components/NihCdeBrowserPanel.tsx'
import { CedarArtifactPanel } from './components/CedarArtifactPanel.tsx'
import { JaxPhenomeBrowserPanel } from './components/JaxPhenomeBrowserPanel.tsx'
import { ImpcBrowserPanel } from './components/ImpcBrowserPanel.tsx'
import { PreclinicalTrialsProtocolPanel } from './components/PreclinicalTrialsProtocolPanel.tsx'
...
const inboundSyncCapabilities: Partial<Record<AppCode, SyncCapability>> = {
    softmouse: {
        label: 'Import SoftMouse Data',
        description: 'Pulls colony data from SoftMouse into Metadatapp.',
        imports: ['Cages', 'Subjects', 'Studies'],
    },
    elabftw: {
...
const appSpecificNotes: Partial<Record<AppCode, string[]>> = {
    ...
    cedar: ['Work with CEDAR artifacts (templates, elements, fields, instances) in the artifact panel below — fetch, create, update, delete and validate, plus import a template into Metadatapp. Mirrors the cedar-artifact-rest-mcp surface.'],
    ...
    nih_cde: ['Browse and import NIH Common Data Elements using the CDE browser below. Imported CDEs are stored locally and can later be linked to form fields.'],
    jax_phenome: ['Browse curated mouse phenotype measures and strains using the Phenome browser below. This integration is read-only — no import is performed.'],
    impc: ['Search standardized IMPReSS phenotyping procedures and parameters for behavioral studies using the browser below. This integration is read-only — no import is performed.'],
}
...
export function ConnectedAppDetailPage() {
    ...
    const syncMutation = useMutation({
        ...
        onSuccess: () => {
            const isImport = data?.code ? Boolean(inboundSyncCapabilities[data.code]) : false
            toast({
                title: isImport ? 'Import queued' : 'Synchronization queued',
                description: 'The task will load records when the connected-app worker is running.',
            })
    ...
    const testNihCdeConnectionMutation = useMutation({
        mutationFn: () => testNihCdeConnection(appId!),
        onSuccess: (result) => {
            toast({
                title: 'Connection valid',
                description: `Connected to NIH CDE API — ${result.cdesCount.toLocaleString()} CDEs reachable.`,
            })
        },
        onError: (err: Error) => {
            toast({
    ...
    const testJaxPhenomeConnectionMutation = useMutation({
        mutationFn: () => testJaxPhenomeConnection(appId!),
        onSuccess: (result) => {
            toast({
                title: 'Connection valid',
                description: `Connected to ${result.externalUrl} — ${result.strainsCount.toLocaleString()} strain(s) reachable.`,
            })
        },
        onError: (err: Error) => {
            toast({
    ...
    const testImpcConnectionMutation = useMutation({
        mutationFn: () => testImpcConnection(appId!),
        onSuccess: (result) => {
            toast({
                title: 'Connection valid',
                description: `Connected to ${result.externalUrl} — ${result.pipelinesCount.toLocaleString()} pipeline(s) reachable.`,
            })
        },
        onError: (err: Error) => {
            toast({
    ...
    const isNihCde = data.code === 'nih_cde'
    const isJaxPhenome = data.code === 'jax_phenome'
    const isImpc = data.code === 'impc'
    // Catalog entries have id === code (not yet persisted to DB).
    // Proxy calls require a real UUID, so we block them until the app is saved.
    const isCatalogEntry = data.id === data.code
        ...
        <div className="space-y-6">
            ...
            <div className="grid gap-6 lg:grid-cols-3">
                <div className="lg:col-span-2 space-y-6">
                                ...
                                <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                                    {data.stats.animalsCount !== undefined && (
                                        <div className="rounded-2xl bg-surface-2 p-5 border border-line shadow-sm">
                                            ...
                                            <p className="mt-1 text-xs text-muted">Total colony size in SoftMouse</p>
                                        </div>
                                    )}
                    ...
                    {isPreclinicalTrials && (
                        <PreclinicalTrialsProtocolPanel appId={appId!} />
                    )}
                            ...
                            tabs={[
                                {
                                    id: 'import',
                                    label: 'Import CSV',
                                    content: <SoftMouseImportTab appId={appId!} />,
                                },
                            ]}
                        />
                    )}
                        ...
                        <Card className="border-indigo-100 bg-indigo-50/30">
                            <div className="flex flex-col gap-5 md:flex-row md:items-start md:justify-between">
                                <div className="space-y-3">
                                    <div>
                                        <h3 className="font-display text-lg font-semibold text-indigo-950">DVC Analytics Workspace</h3>
                                        <p className="mt-1 text-sm text-slate-600">
                                            Use Analytics Hub to extract Tecniplast data, reopen completed jobs, and move directly into visual analysis.
                                        </p>
                                    </div>
                                    <div className="flex flex-wrap gap-2">
                                        <Badge variant="outline">Live API extraction</Badge>
                                        <Badge variant="outline">Saved task history</Badge>
                                    ...
                                    <div className="grid gap-3 sm:grid-cols-3">
                                        <div className="rounded-xl border border-indigo-100 bg-white/80 p-3">
                                            <Activity className="mb-2 h-4 w-4 text-indigo-600" />
                                            <p className="text-xs font-semibold text-slate-900">Time-series plots</p>
                                            <p className="mt-1 text-xs text-slate-500">Raw, 15 min, hourly, and daily bins.</p>
                                        </div>
                                        <div className="rounded-xl border border-indigo-100 bg-white/80 p-3">
                                            <Boxes className="mb-2 h-4 w-4 text-indigo-600" />
                                            <p className="text-xs font-semibold text-slate-900">Cohort statistics</p>
                                            <p className="mt-1 text-xs text-slate-500">Groups, distributions, PCA, and cosinor fits.</p>
                                        </div>
                                        <div className="rounded-xl border border-indigo-100 bg-white/80 p-3">
                                            <Database className="mb-2 h-4 w-4 text-indigo-600" />
                                            <p className="text-xs font-semibold text-slate-900">Reusable exports</p>
                                            <p className="mt-1 text-xs text-slate-500">Download raw ZIPs or analysis CSVs.</p>
                                        </div>
                                    </div>
                                </div>
                                <Button asChild className="shrink-0">
                                    <Link to="/dvc/analytics">
                    ...
                    {isNihCde && (
                        <Card className="bg-surface-3 border-line">
                            <h3 className="font-display text-lg font-semibold text-ink mb-5">NIH CDE Browser</h3>
                            {isCatalogEntry ? (
                                <div className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-6 text-center space-y-3">
                                    <p className="text-sm font-medium text-amber-900">
                                        Activate this integration to start browsing CDEs.
                                    </p>
                                    <p className="text-xs text-amber-700">
                                        Click <strong>Edit Settings</strong> in the sidebar and save the default settings. No credentials are required — the NIH CDE repository is public.
                                    </p>
                                </div>
                            ) : (
                                <NihCdeBrowserPanel appId={appId!} />
                            )}
                        </Card>
                    )}
                    ...
                    {isCedar && (
                        <Card className="bg-surface-3 border-line">
                            <h3 className="font-display text-lg font-semibold text-ink mb-5">CEDAR Artifacts</h3>
                            {isCatalogEntry ? (
                                <div className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-6 text-center space-y-3">
                                    <p className="text-sm font-medium text-amber-900">
                                        Activate this integration to work with CEDAR artifacts.
                                    </p>
                                    <p className="text-xs text-amber-700">
                                        Click <strong>Edit Settings</strong> in the sidebar, enter your CEDAR API key (and optional URL), and save.
                                    </p>
                                </div>
                            ) : (
                                <CedarArtifactPanel appId={appId!} />
                            )}
                        </Card>
                    )}
                    ...
                    {isJaxPhenome && (
                        <Card className="bg-surface-3 border-line">
                            <h3 className="font-display text-lg font-semibold text-ink mb-5">JAX Phenome Browser</h3>
                            {isCatalogEntry ? (
                                <div className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-6 text-center space-y-3">
                                    <p className="text-sm font-medium text-amber-900">
                                        Activate this integration to start browsing phenotype measures.
                                    </p>
                                    <p className="text-xs text-amber-700">
                                        Click <strong>Edit Settings</strong> in the sidebar and save the default settings. No credentials are required — the Mouse Phenome Database is public.
                                    </p>
                                </div>
                            ) : (
                                <JaxPhenomeBrowserPanel appId={appId!} />
                            )}
                        </Card>
                    )}
                    ...
                    {isImpc && (
                        <Card className="bg-surface-3 border-line">
                            <h3 className="font-display text-lg font-semibold text-ink mb-5">IMPReSS Standardized Resources</h3>
                            {isCatalogEntry ? (
                                <div className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-6 text-center space-y-3">
                                    <p className="text-sm font-medium text-amber-900">
                                        Activate this integration to start searching standardized procedures.
                                    </p>
                                    <p className="text-xs text-amber-700">
                                        Click <strong>Edit Settings</strong> in the sidebar and save the default settings. No credentials are required — the IMPReSS catalogue is public.
                                    </p>
                                </div>
                            ) : (
                                <ImpcBrowserPanel appId={appId!} />
                            )}
                        </Card>
                    )}
                ...
                <div className="space-y-6">
                    <Card className="bg-surface-3 border-line shadow-panel">
                        ...
                        <div className="mt-8 space-y-3">
                            {supportsInboundSync && !isPreclinicalTrials && (
                                <Button
                                    className="w-full"
                            ...
                            {isNihCde && !isCatalogEntry && (
                                <Button
                                    variant="outline"
                                    className="w-full"
                                    onClick={() => testNihCdeConnectionMutation.mutate()}
                                    disabled={testNihCdeConnectionMutation.isPending}
                                >
                                    {testNihCdeConnectionMutation.isPending ? 'Testing...' : 'Test Connection'}
                                </Button>
                            )}
                            {isJaxPhenome && !isCatalogEntry && (
                                <Button
                                    variant="outline"
                                    className="w-full"
                                    onClick={() => testJaxPhenomeConnectionMutation.mutate()}
                                    disabled={testJaxPhenomeConnectionMutation.isPending}
                                >
                                    {testJaxPhenomeConnectionMutation.isPending ? 'Testing...' : 'Test Connection'}
                                </Button>
                            )}
                            {isImpc && !isCatalogEntry && (
                                <Button
                                    variant="outline"
                                    className="w-full"
                                    onClick={() => testImpcConnectionMutation.mutate()}
                                    disabled={testImpcConnectionMutation.isPending}
                                >
                                    {testImpcConnectionMutation.isPending ? 'Testing...' : 'Test Connection'}
                                </Button>
                            )}
                            {data.externalUrl && (
