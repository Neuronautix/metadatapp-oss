...
function all(): int
{
    ...
    return max($agentDocs, $docsCheck, $cs, $phpstan, $phpunit, $schemaValidate);
}
...
#[AsTask(description: 'Checks Connected Apps user-guide pages match the AppCode enum', aliases: ['docs-check'])]
function docs_check(): int
{
    io()->section('Checking Connected Apps documentation coverage...');

    $process = run(
        ['python3', variable('root_dir') . '/scripts/check_connected_apps_docs.py'],
        context: context()->withAllowFailure(),
    );
