...
function about(): void
{
    ...
    $extraDomains = variable('extra_domains');
    $osomaDomain = $extraDomains[0] ?? "osoma.{$rootDomain}";
    $authDomain = $extraDomains[1] ?? "auth.{$rootDomain}";

    io()->listing([
        \sprintf('API: https://%s', $rootDomain),
        \sprintf('Docs: https://%s/docs', $rootDomain),
        \sprintf('Admin: https://%s/admin', $rootDomain),
        \sprintf('Profiler: https://%s/_profiler', $rootDomain),
        \sprintf('Osoma: https://%s', $osomaDomain),
        \sprintf('Auth: https://%s', $authDomain),
        \sprintf('Keycloak: https://%s/oidc/', $authDomain),
    ]);
}
...
function docker_compose(array $subCommand, ?Context $c = null, bool $withBuilder = false, array $profiles = []): Process
{
    ...
    $domains = 'Host(`' . implode('`) || Host(`', $domains) . '`)';
    ...
    // When running inside a devcontainer, the Docker daemon resolves bind-mount paths on the
    // HOST filesystem, not the devcontainer's. HOST_PROJECT_ROOT is injected by devcontainer.json
    // via ${localWorkspaceFolder}. We expose it as PROJECT_HOST_ROOT so compose files can use
    // ${PROJECT_HOST_ROOT:-<relative-fallback>} in volume definitions.
    if ($hostRoot = \getenv('HOST_PROJECT_ROOT')) {
        $c = $c->withEnvironment(['PROJECT_HOST_ROOT' => $hostRoot]);
    }
