...
final readonly class AiAssistantService
{
    public function __construct(
        ...
        private ToolUseOrchestrator $toolUseOrchestrator,
    ...
    /**
     * Agentic chat: runs a bounded multi-step tool-use loop over the approved MCP
     * tools when AI_AGENTIC_MODE_ENABLED is on. Degrades to the single-shot
     * {@see self::chat()} when the flag is off, so the endpoint is always safe.
     */
    public function agenticChat(AiChatRequest $request): AiChatResponse
    {
        if (!$this->featureFlags->isAgenticModeEnabled()) {
            // Graceful degradation: flag off -> existing single-shot behaviour.
            return $this->chat($request);
        }

        $mappedTask = $this->taskMapper->mapAgenticChat($request);
        $this->guardTask($mappedTask->request->taskType);

        $result = $this->toolUseOrchestrator->run($this->currentGateway(), $mappedTask->request);
        ...
        $this->logger->info('AI action completed', [
            ...
            'agentic_iteration_cap_reached' => $result->iterationCapReached,
            'agentic_tool_calls' => \count($result->toolProvenance),
            'warning_count' => \count($response->warnings) + \count($result->warnings),
        ]);
        ...
        return new AiChatResponse(
            ...
            status: $status,
            trace: $trace,
            provenance: [...$result->toolProvenance, ...$response->provenance],
