...
class TecniplastProxyController extends AbstractController
{
    ...
    private function getValidToken(ConnectedApp $connectedApp): string
    {
        ...
        $token = $this->getTecniplastApiKey($connectedApp);
        if (null === $token) {
    ...
    private function getTecniplastApiKey(ConnectedApp $connectedApp): ?string
    {
        $authenticationParameters = $connectedApp->getAuthenticationParameters() ?? [];

        foreach (['accessToken', 'apiKey', 'token'] as $key) {
