...
class ExternalLookupService
{
    ...
    public function search(string $source, string $query): array
    {
        ...
        return match ($source) {
            ...
            'ols_efo' => $this->searchOlsOntology($query, 'efo', 'EFO'),
            'ols_vt' => $this->searchOlsOntology($query, 'vt', 'VT'),
            'ols_mp' => $this->searchOlsOntology($query, 'mp', 'MP'),
            'ols_ma' => $this->searchOlsOntology($query, 'ma', 'MA'),
    ...
    /**
     * Generic OLS ontology lookup (EFO, VT, MP, MA, …). NCBITaxon keeps its own
     * species-aware mapping in {@see searchNcbiTaxon()}.
     *
     * @return list<array{label: string, sublabel: ?string, value: string, externalId: ?string, scheme: string}>
     */
    private function searchOlsOntology(string $query, string $ontology, string $scheme): array
    {
        $payload = $this->httpClient->request('GET', self::OLS_URL, [
            'timeout' => self::LOOKUP_TIMEOUT_SECONDS,
            'max_duration' => self::LOOKUP_MAX_DURATION_SECONDS,
