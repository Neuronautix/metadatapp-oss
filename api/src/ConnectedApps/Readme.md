...
This module handles integrations with external applications such as Fair3R, eLabFTW, OSF, protocols.io, Tecniplast, and sensor agents. Each external system is represented by a `ConnectedApp` entity identified by an `AppCode` enum value. Services implementing the synchronization logic are auto-discovered through dependency injection.
