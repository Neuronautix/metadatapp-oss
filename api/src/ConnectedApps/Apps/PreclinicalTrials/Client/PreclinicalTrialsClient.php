...
final class PreclinicalTrialsClient implements PreclinicalTrialsClientInterface
{
    ...
    public function getBaseUrl(ConnectedApp $connectedApp): string
    {
        ...
        $baseUrl = null === $externalUrl || '' === trim($externalUrl) ? self::DEFAULT_API_URL : rtrim(trim($externalUrl), '/');

        return preg_replace(
            '#^https?://www\.preclinicaltrials\.eu(?=/|$)#i',
