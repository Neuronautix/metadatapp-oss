...
> the release backlog (which describes what's *next*).
...
- Failure isolation: a broken SoftMouse adapter doesn't take down
  `/experiments`.
...
- **Adapter pattern in `ConnectedApps/`.** The
  ...
  across `Elabftw`, `SoftMouse`, `Tecniplast`, `Fair3r`. That
